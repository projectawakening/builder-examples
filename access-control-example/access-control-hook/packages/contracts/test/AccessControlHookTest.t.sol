...
// AccessControlHook and hook test dependency imports
import { IAccessControlHookErrors } from "../src/interfaces/IAccessControlHookErrors.sol";
...
import { IUnapprovedForwarderSystem } from "../src/interfaces/IUnapprovedForwarderSystem.sol";
import { IPermissionedSystem } from "../src/interfaces/IPermissionedSystem.sol";
import { AccessControlHook } from "../src/systems/AccessControlHook.sol";
import { ForwarderSystem } from "../src/systems/ForwarderSystem.sol";
import { UnapprovedForwarderSystem } from "../src/systems/UnapprovedForwarderSystem.sol";
...
import { ADMIN, APPROVED, EVE_WORLD_NAMESPACE, NAMESPACE, ACCESS_ROLE_TABLE_NAME, ACCESS_CONTROL_HOOK_SYSTEM_NAME } from "../src/constants.sol";
...
contract AccessControlHookTest is Test {
  ...
  ResourceId UNAPPROVED_FORWARDER_SYSTEM_ID = WorldResourceIdLib.encode({
    typeId: RESOURCE_SYSTEM,
    namespace: NAMESPACE,
  ...
  UnapprovedForwarderSystem unapprovedForwarder;
  ...
  function setUp() public {
    ...
    // Unapproved Forwarder deploy, then System and functions EVE World reigster
    unapprovedForwarder = new UnapprovedForwarderSystem();
    world.registerSystem(UNAPPROVED_FORWARDER_SYSTEM_ID, System(unapprovedForwarder), true);
    world.registerFunctionSelector(UNAPPROVED_FORWARDER_SYSTEM_ID, "rejectedCallOnlyApprovedForwarderPermissioned(uint256)");
    ...
    // REGISTER AccessRole Table beofre AccessControlHook (becuase AccessControlHook uses it) 
    AccessRole.register();
    ...
    // AccessControlHook deploy, then System and functions EVE World register
    AccessControlHook accessControlHook = new AccessControlHook();
    world.registerSystem(ACCESS_CONTROL_HOOK_SYSTEM_ID, System(accessControlHook), true);
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "onlyAdminRoleTxOrigin(uint256)");
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "onlyOwnerInitialMsgSender(uint256)");
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "onlyApprovedRoleForwardedMsgSender(uint256)");
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "storeSystemAddress(uint256)");
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "getStoredAddress(uint256)");
    world.registerFunctionSelector(ACCESS_CONTROL_HOOK_SYSTEM_ID, "setAccessListByRole(bytes32,address[])");
    ...
    // START: our custom module configuration
    ...
    SOFInterface.registerHook(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyAdminRoleTxOrigin.selector);
    uint256 onlyAdminHookId = uint256(keccak256(abi.encodePacked(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyAdminRoleTxOrigin.selector)));
    SOFInterface.registerHook(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyOwnerInitialMsgSender.selector);
    uint256 onlyOwnerHookId = uint256(keccak256(abi.encodePacked(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyOwnerInitialMsgSender.selector)));
    SOFInterface.registerHook(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyApprovedRoleForwardedMsgSender.selector);
    uint256 onlyApprovedHookId = uint256(keccak256(abi.encodePacked(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.onlyApprovedRoleForwardedMsgSender.selector)));
    SOFInterface.registerHook(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.storeSystemAddress.selector);
    uint256 storeSystemHookId = uint256(keccak256(abi.encodePacked(ACCESS_CONTROL_HOOK_SYSTEM_ID, IAccessControlHook.storeSystemAddress.selector)));

    // CONFIGURE HOOKS in the SOF
    SOFInterface.addHook(storeSystemHookId, HookType.BEFORE, FORWARDER_SYSTEM_ID, IForwarderSystem.callOnlyApprovedForwarderPermissioned.selector);
    SOFInterface.addHook(storeSystemHookId, HookType.BEFORE, UNAPPROVED_FORWARDER_SYSTEM_ID, IUnapprovedForwarderSystem.rejectedCallOnlyApprovedForwarderPermissioned.selector);

    SOFInterface.addHook(onlyAdminHookId, HookType.BEFORE, PERMISSIONED_SYSTEM_ID, IPermissionedSystem.onlyAdminPermissioned.selector);
    SOFInterface.addHook(onlyOwnerHookId, HookType.BEFORE, PERMISSIONED_SYSTEM_ID, IPermissionedSystem.onlyOwnerPermissioned.selector);
    SOFInterface.addHook(onlyApprovedHookId, HookType.BEFORE, PERMISSIONED_SYSTEM_ID, IPermissionedSystem.onlyApprovedForwarderPermissioned.selector);

    // ASSOCIATE HOOKS with our dummy SSU ID in SOF
    SOFInterface.associateHook(ssuId, storeSystemHookId);
    SOFInterface.associateHook(ssuId, onlyAdminHookId);
    SOFInterface.associateHook(ssuId, onlyOwnerHookId);
    SOFInterface.associateHook(ssuId, onlyApprovedHookId);

    // END: our custom module configuration
  }
  ...
  function testRoleSetPass() public {
    ...
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (ADMIN, adminAccessList)));
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (APPROVED, approvedAccessList)));
    vm.stopPrank();
  ...
  function testRoleSetFailByRoleId() public {
    ...
    vm.prank(deployer);
    vm.expectRevert();
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (bytes32("OWNER"), ownerAccessList)));
  }
  ...
  function testOnlyAdminRejection() public {
    ...
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (ADMIN, adminAccessList)));
    vm.expectRevert(
      abi.encodeWithSelector(IAccessControlHookErrors.AccessControl_NoPermission.selector, alice, bytes32(ADMIN))
    );
    world.call(PERMISSIONED_SYSTEM_ID, abi.encodeCall(IPermissionedSystem.onlyAdminPermissioned, (ssuId)));
    vm.stopPrank();
  }
  ...
  function testOnlyOwnerRejection() public {
    vm.startPrank(bob, deployer);
        vm.expectRevert(
      abi.encodeWithSelector(IAccessControlHookErrors.AccessControl_NoPermission.selector, bob, bytes32("OWNER"))
    );
    world.call(PERMISSIONED_SYSTEM_ID, abi.encodeCall(IPermissionedSystem.onlyOwnerPermissioned, (ssuId)));
    vm.stopPrank();
  }

  function testOnlyApprovedForwarderPass1() public {
    address[] memory approvedAccessList = new address[](1);
    approvedAccessList[0] = address(forwarder);
    vm.startPrank(deployer);
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (APPROVED, approvedAccessList)));
  ...
  function testOnlyApprovedForwarderRejectionNoForwarder() public {
    address[] memory approvedAccessList = new address[](1);
    approvedAccessList[0] = address(forwarder);
    vm.startPrank(deployer);
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (APPROVED, approvedAccessList)));
    vm.expectRevert(
      abi.encodeWithSelector(IAccessControlHookErrors.AccessControl_NoPermission.selector, address(0), APPROVED)
    );
    world.call(PERMISSIONED_SYSTEM_ID, abi.encodeCall(IPermissionedSystem.onlyApprovedForwarderPermissioned, (ssuId)));
    vm.stopPrank();
  }

  function testOnlyApprovedForwarderRejectionUnapprovedForwarder() public {
    address[] memory approvedAccessList = new address[](1);
    approvedAccessList[0] = address(forwarder);
    ...
    world.call(ACCESS_CONTROL_HOOK_SYSTEM_ID, abi.encodeCall(IAccessControlHook.setAccessListByRole, (APPROVED, approvedAccessList)));
    vm.expectRevert(
      abi.encodeWithSelector(IAccessControlHookErrors.AccessControl_NoPermission.selector, address(unapprovedForwarder), APPROVED)
    );
    world.call(UNAPPROVED_FORWARDER_SYSTEM_ID, abi.encodeCall(IUnapprovedForwarderSystem.rejectedCallOnlyApprovedForwarderPermissioned, (ssuId)));
    vm.stopPrank();
  }
