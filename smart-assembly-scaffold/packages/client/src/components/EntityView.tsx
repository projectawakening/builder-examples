  ...
  return (
    <div className="grid gap-4 grid-cols-1 mobile:px-5">
      ...
      <div>
        ...
        <span className="underline font-bold">{smartAssembly?.name || abbreviateAddress(smartAssembly?.id)}</span>
      </div>

      <div>
        Description:<div>{smartAssembly?.description || "No description set"}</div>
      </div>
