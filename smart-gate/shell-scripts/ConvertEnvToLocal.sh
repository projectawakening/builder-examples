...
TEST_PLAYER_PRIVATE_KEY=$(grep '^TEST_PLAYER_PRIVATE_KEY=' "$ENV_FILE" | cut -d '=' -f2-)
...
$SED_CMD $SED_OPTS "s/^PRIVATE_KEY=.*/PRIVATE_KEY=$TEST_PLAYER_PRIVATE_KEY/" "$ENV_FILE"
...
printf "${GREEN}[COMPLETED]${RESET} Set ${YELLOW}PRIVATE_KEY${RESET} in ${YELLOW}.env${RESET} to ${YELLOW}[$TEST_PLAYER_PRIVATE_KEY]${RESET}\n\n"
