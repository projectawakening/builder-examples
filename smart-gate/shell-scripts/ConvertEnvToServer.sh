ENV_FILE=".env"
STILLNESS_PRIVATE_KEY=$(grep '^STILLNESS_PRIVATE_KEY=' "$ENV_FILE" | cut -d '=' -f2-)
...
$SED_CMD $SED_OPTS "s/^PRIVATE_KEY=.*/PRIVATE_KEY=$STILLNESS_PRIVATE_KEY/" "$ENV_FILE"
...
printf "${GREEN}[COMPLETED]${RESET} Set ${YELLOW}PRIVATE_KEY${RESET} in ${YELLOW}.env${RESET} to ${YELLOW}[STILLNESS_PRIVATE_KEY]${RESET}\n\n"