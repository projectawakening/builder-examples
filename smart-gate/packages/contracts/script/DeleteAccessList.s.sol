...
contract DeleteAccessList is Script {
  ...
  function run(address worldAddress, bytes32 accesslistId) external {
    ...
    world.call(
      systemId,
      abi.encodeCall(
        SmartGateSystem.deleteAccessList,
        (accesslistId)
      )
    );

    console.log("Deleted access list with ID");
