...
contract RemoveAccessListFromGate is Script {
  using SmartGateUtils for bytes14;
  using SmartGateLib for SmartGateLib.World;

  SmartGateLib.World smartGate;

  function run(address worldAddress, uint256 gateId, bytes32 accessListId) external {
    ...
    console.log("access list ID");
    console.logBytes32(accessListId);
    console.log("removed from gate ID ", gateId);
