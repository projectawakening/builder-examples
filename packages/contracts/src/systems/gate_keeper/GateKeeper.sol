...
contract GateKeeper is System {
  ...
  function depositToSSU(uint256 smartObjectId, uint256 quantity) public {
    ...
    require(gatekeeperConfig.isGoalReached == false, "GateKeeper not set");
    ...
    if (entityInRecord.recordExists == false) {
      revert IInventoryErrors.Inventory_InvalidItem("ItemSeller: item is not created on-chain", entityInRecord.itemId);
    }
